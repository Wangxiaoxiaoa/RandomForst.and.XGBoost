...
for i in range(10):
    ...
    xgb = XGB(base_score=0.5,max_depth=i+1,n_estimators=4,learning_rate=0.3,reg_lambda=1,
    ...
    print('max_depth为{}时，训练集f1_score为{}：'.format(i + 1, train_f1_score))
    # 测试集评估
    test_predict = xgb.predict_prob(x_test)
    test_f1_score = f1_score(y_test, test_predict)
    test_f1.append(test_f1_score)
    print('max_depth为{}时，测试集f1_score为{}：'.format(i + 1, test_f1_score))
...
plt.ylabel('f1_score')
plt.title('max_depth and f1_score')
plt.show()