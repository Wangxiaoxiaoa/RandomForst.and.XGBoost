...
for i in range(20):
    print('n_estimators为{}:'.format(i+1))
    ...
    print('n_estimators为{}时，训练集r2_score为{}：'.format(i+1,train_r2_score))
    #测试集评估
    test_predict = xgb.predict_raw(x_test)
    test_r2_score = r2_score(y_test, test_predict)
    test_r2.append(test_r2_score)
    print('n_estimators为{}时，测试集r2_score为{}：'.format(i+1,test_r2_score))
...
plt.ylabel('r2_score')
plt.title('n_estimators and r2_score')
plt.show()