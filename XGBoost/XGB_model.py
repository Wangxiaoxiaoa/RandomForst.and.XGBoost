...
class XGB:
    ...
    def __init__(self,
        ...
        self.objective = objective  # 目标函数，可选regression和classifier
    ...
    def xgb_cart_tree(self, X, w, m_dpth):
        ...
        for item in [x for x in X.columns if x not in ['g', 'h', 'y']]:
            for cut in list(set(X[item])):
                ...
                # 当目标函数为regression，即1/2*(y-y_hat)**2时，它的二阶导是1，那min_child_weight就等价于min_child_sample
                # 当目标函数为classifier，其二阶导为sigmoid(y_hat)*(1-sigmoid(y_hat))，可理解为叶子节点的纯度，更详尽的解释可参看：
    ...
    def _grad(self, y_hat, Y):
        ...
        if self.objective == 'classifier':
            ...
            raise KeyError('objective must be regression or classifier!')
    ...
    def predict_prob(self, X: pd.DataFrame):
        '''
        当指定objective为classifier时，输出概率要做一个classifier转换
        '''
